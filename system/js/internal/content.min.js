...
($(".mce-toolbar-grp").show(),$(".above-editor-links").removeClass("grid_4").addClass("grid_8"),$("#tabs").removeClass("grid_8").addClass("grid_4"))});$(document).on("click",'[aria-label="Fullscreen"]',function(){$(window.parent.document).find('[aria-describedby="toasterPopupDraggable"]').toggleClass("screen-expand");var a=$("#tabs"),b=a.height(),e=a.find(".ui-tabs-nav").height(),c=a.find("#adminthingsviewer .ui-accordion-header"),d=c.length,c=c.outerHeight(),f=a.find("#adminselectimgfolder").outerHeight(),
g=a.find("#btn-create").outerHeight(),h=a.find("#widgetSync").outerHeight()+5;a.find("#adminthingsviewer .ui-accordion-content").css({"max-height":b-e-(c+2)*d-f-30});a.find("#product-products").css({height:b-e-g-116});a.find(".netcontent-widget-list").css({height:b-e-h-12})});$("#btn-submit").click(function(){$("#frm_content").submit()});$("#frm_content").submit(function(){var a={content:$(this).find("#content").val(),containerType:$(this).find("#container_type").val(),containerName:$(this).find("#container_name").val(),
...
function(){tinymce.activeEditor.setContent(c);$("#content").val(c)},function(){localStorage.removeItem(generateStorageKey())},"success");$(document).on("click","span.refresh-items",function(a){a.preventDefault();$("#adminselectimgfolder").trigger("change")});$(document).on("click","#generate-ai-content-text",function(a){a=0;var b=tinymce.activeEditor.getContent();a=parseInt($(document).find("#ai-content-amount").val());showSpinner();$.ajax({url:$("#website_url").val()+"api/toaster/containersai/",
type:"POST",dataType:"json",data:{content:b,wordCount:a,pageId:$(document).find("#container-page-id").val(),secureToken:$(document).find("#container-secure-token").val()}}).done(function(a){hideSpinner();if(1===parseInt(a.error))return showMessage(a.message,!0,3E3),!1;tinymce.activeEditor.setContent(a.message)})})});function dispatchEditorKeyup(c,d,a){var b=a;null===b&&(b=setTimeout(function(){localStorage.setItem(generateStorageKey(),tinymce.activeEditor.getContent());b=null},1E3))}
